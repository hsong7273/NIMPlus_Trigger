...
//		inet_ntop(AF_INET, &cli_addr, buf, sizeof(buf));
//		printf("address:%s/n", buf);
//		newsockfd = accept(sockfd. sockaddr*);
		if (newsockfd < 0)
...
//		printf("client address: %c \n", *p->clientaddr);
        ...
        if (p->updateParams == 1) {
			 ...
			 // Write the deadtime
			 writeGPIO(deadtime_addr, p->deadtime, fd);
			 // Write the output pulse width
			 writeGPIO(pulsewidth_addr, p->pulsewidth, fd);
